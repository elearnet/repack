...
//  Pods
//
//  Created by eadron.lin on 7/17/25.
//
import Foundation
import CommonCrypto
import JWTDecode
...
public class CodeSigningUtils: NSObject {
    ...
    private static func decodeAndVerifyToken(token: String, publicKey: String) throws -> JWTDecode.JWT {
        ...
        guard let signedData = token.components(separatedBy: ".").dropLast().joined(separator: ".").data(using: .utf8) else {
        ...
        // 3. Create a CryptoKit key from the PEM string
        let p256Key = try createPublicKey(from: publicKey)
        let signature = try P256.Signing.ECDSASignature(rawRepresentation: signatureData)
        
        
        // 4. Verify the signature against the signed data
        // This is the replacement for the SwiftyRSA.verify() call
        let isSuccessfullyVerified = p256Key.isValidSignature(signature, for: signedData)
    ...
    private static func createPublicKey(from pemString: String) throws -> P256.Signing.PublicKey {
        // Clean up the PEM string by removing header, footer, and newlines
        ...
        // Create a CryptoKit public key from the raw data
        return try P256.Signing.PublicKey(rawRepresentation: keyData)
    }
